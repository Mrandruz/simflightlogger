...
const regexToReplace = /    const prompt = `Sei ARIA, il sistema operativo di Velar Airlines\.[\s\S]+?Rispondi SOLO con JSON valido, nessun testo extra, nessun markdown:`/g;

let match = ariaCode.match(regexToReplace);
if (match) {
...
Rispondi SOLO con JSON valido, nessun testo extra, nessun markdown:\``;
    ...
    console.log('ARIAAssistant.tsx updated successfully');
} else {
