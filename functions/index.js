...
const buildPrompt = (s) => [
  ...
  "1. compressedLogbook: the pilot's COMPLETE flight history as CSV rows",
  "   (date,departure,arrival,aircraft,airline,flightTime_h,distance_nm).",
  "   This is the ground truth. Use it for ANY ranking, count, filter or",
  "   detailed analysis — top airports, top routes, per-year breakdowns,",
  "   per-aircraft stats, date ranges, etc. Always derive rankings from",
  "   this data directly; never cap results unless the user asks you to.",
  "2. Aggregated stats fields (totalFlights, topAircraftList, etc.):",
  "   use these for quick context and summaries only.",
  "",
  "RULES:",
  "- When the user asks for a top-N list, compute it from compressedLogbook.",
  "- Never say you only have data for top 5 — the full logbook is available.",
  "- If compressedLogbook is empty or missing, fall back to aggregated stats",
  "  and tell the user the logbook detail is unavailable.",
  "",
  ...
  "",
  "── AGGREGATED STATS (quick reference) ──",
  ...
  `Most used aircraft: ${s.topAircraft || "—"}`,
  `Hub airport: ${s.topAirport || "—"}`,
  `Most used airline: ${s.topAirline || "—"}`,
  `Favourite route: ${s.topRoute || "—"}`,
  `Flights last 30 days: ${s.flightsLastMonth ?? "—"}`,
  `Average hours/flight: ${s.avgHours || "—"}h`,
  `Longest flight: ${s.longestFlight || "—"}`,
  `Top 20 aircraft: ${s.topAircraftList || "—"}`,
  `Top 20 airports: ${s.topAirportList || "—"}`,
  `Top 20 routes: ${s.topRouteList || "—"}`,
  `Monthly trend (last 6 months): ${s.monthlyDistribution || "—"}`,
  ...
  "",
  "── PER-AIRCRAFT LOGBOOK (flights, hours, nm, last flight, top route) ──",
  s.aircraftLogbook || "no data",
  "",
  "── COMPLETE FLIGHT LOGBOOK (ground truth — CSV format) ──",
  "date,departure,arrival,aircraft,airline,flightTime_h,distance_nm",
  s.compressedLogbook || "no data",
].join("\n").trim();
